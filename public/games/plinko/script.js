...
class Ball {
    constructor(x, y, betValue) {
        ...
        // Blockchain guidance properties
        this.guidedDrop = false;
    ...
    update() {
        // 🎯 BLOCKCHAIN GUIDED DROP
        if (this.guidedDrop && this.active) {
            // Apply gravity
            this.vel.y += GAME_CONFIG.gravity;
            
            // Gently guide ball toward target X position
            const distToTarget = this.targetX - this.pos.x;
            
            // Apply horizontal correction force (subtle steering)
            if (Math.abs(distToTarget) > 5) {
                const steerForce = distToTarget * 0.015; // Gentle steering
                this.vel.x += steerForce;
            }
            
            // Limit horizontal velocity to keep it natural
            const maxVelX = 4;
            if (this.vel.x > maxVelX) this.vel.x = maxVelX;
            if (this.vel.x < -maxVelX) this.vel.x = -maxVelX;
...
class PlinkoGame {
    ...
    handleWin(ball, slot) {
        ...
        // Only call ui.onWin if NOT a blockchain guided drop
        if (!ball.guidedDrop) {
...
class UIController {
    ...
    bindEvents() {
        ...
        document.getElementById('dailyBonusBtn').onclick = () => {
             ...
             setTimeout(() => { document.getElementById('dailyBonusBtn').disabled = false; }, 60000);
        };
...
let pendingBlockchainBet = false;

if (window.parent !== window) {
    blockchainMode = true;
    console.log('🔗 Blockchain mode enabled - Running inside iframe');
    
    window.parent.postMessage({ type: 'PLINKO_READY' }, '*');
    ...
    if (panelHeader) {
        const blockchainBalance = document.createElement('div');
        blockchainBalance.id = 'blockchainBalance';
        blockchainBalance.style.cssText = 'background: rgba(0,136,255,0.2); padding: 8px 12px; border-radius: 8px; font-size: 0.9rem; border: 1px solid rgba(0,136,255,0.4);';
        blockchainBalance.innerHTML = '<span style="color: #0088FF;">⚡</span> <span style="font-weight: bold;">Blockchain Mode</span>';
        panelHeader.appendChild(blockchainBalance);
    }
}

// 🎯 BLOCKCHAIN: Drop ball to exact position from blockchain
function dropBallToBlockchainPosition(finalPosition, betAmount, rows, risk) {
    console.log(`🎯 Dropping ball to position ${finalPosition} (${rows} rows, ${risk} risk)`);
    
    const ball = new Ball(ui.game.width / 2, 0, betAmount);
    
    // Calculate target slot position
    const gap = ui.game.width / (ui.game.rowCount + 2);
    const slotY = 30 + ((ui.game.rowCount - 1) * gap) + gap;
    const multipliers = GAME_CONFIG.multipliers[rows][risk];
    const targetX = (ui.game.width / 2) - ((multipliers.length) * gap / 2) + (finalPosition * gap) + (gap/2);
    
    // Enable guided drop
    ball.guidedDrop = true;
    ball.targetSlot = finalPosition;
    ball.targetX = targetX;
    ball.targetY = slotY;
    
    ui.game.balls.push(ball);
}

window.addEventListener('message', (event) => {
    if (event.data.type === 'BLOCKCHAIN_RESULT') {
        const { won, multiplier, payout, finalPosition, betAmount } = event.data.data;
        
        console.log('📊 Blockchain Result:', { won, multiplier, payout, finalPosition });
        
        // 🔥 DROP BALL TO EXACT BLOCKCHAIN POSITION
        const risk = ui.game.risk;
        const rows = ui.game.rowCount;
        dropBallToBlockchainPosition(finalPosition, betAmount, rows, risk);
        
        // Wait for ball to land, then show result
        setTimeout(() => {
            // Show payout notification
            ui.showToast(
                won ? `🎉 Won ${payout.toFixed(4)} SUI! (${multiplier}x)` : `Lost ${betAmount.toFixed(4)} SUI`, 
                won ? 'success' : 'error'
            );
        }, 3000);
        
        pendingBlockchainBet = false;
    }
    
    if (event.data.type === 'BET_FAILED') {
        ui.showToast('❌ Bet failed: ' + event.data.error, 'error');
        pendingBlockchainBet = false;
    }
    
    if (event.data.type === 'PLATFORM_BALANCE_UPDATE') {
        const balance = event.data.balance;
        console.log('💰 Platform balance updated:', balance);
    }
});

if (blockchainMode) {
    const originalDrop = ui.drop.bind(ui);
    
    ui.drop = function(count) {
        if (count > 1) {
            ui.showToast('⚠️ Multi-drop not supported in blockchain mode', 'error');
            return;
        }
        
        if (pendingBlockchainBet) {
            ui.showToast('⏳ Please wait for current bet to complete', 'info');
            return;
        }
        
        const betAmountInDollars = parseFloat(document.getElementById('betAmount').value);
        const risk = ui.game.risk;
        const rows = ui.game.rowCount;
        
        if (!betAmountInDollars || betAmountInDollars <= 0) {
            ui.showToast('⚠️ Invalid bet amount', 'error');
            return;
        }
        
        // 🔥 CONVERT DOLLARS TO SUI (1 dollar = 0.01 SUI for demo)
        const betAmount = betAmountInDollars * 0.01; // Adjust conversion rate as needed
        
        console.log('🎲 Requesting blockchain bet:', { 
            betAmountInDollars, 
            betAmountInSUI: betAmount, 
            risk, 
            rows 
        });
        
        pendingBlockchainBet = true;
        
        window.parent.postMessage({
            type: 'PLINKO_BET_REQUEST',
            data: {
                betAmount: betAmount, // Send in SUI
                risk: risk,
                rows: rows
            }
        }, '*');
        
        ui.showToast('⏳ Processing blockchain transaction...', 'info');
    };
    
    const autoBtn = document.getElementById('autoBtn');
    if (autoBtn) {
        autoBtn.disabled = true;
        autoBtn.title = 'Auto-play not available in blockchain mode';
    ...
    document.querySelectorAll('.multi-btn').forEach(btn => {
        ...
        btn.title = 'Multi-drop not available in blockchain mode';
    });
}

console.log('✅ Plinko Blockchain Integration Loaded');