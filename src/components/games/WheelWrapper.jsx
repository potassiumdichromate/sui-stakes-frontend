...
export function WheelWrapper() {
  ...
  useEffect(() => {
    const handleMessage = (event) => {
      if (event.data.type === 'WHEEL_READY') {
        setGameReady(true);
        console.log('✅ Wheel game ready!');
      ...
      if (event.data.type === 'WHEEL_BET_REQUEST') {
  ...
  // Send balance when game is ready
  useEffect(() => {
  ...
  const handleBackendBet = async ({ betAmount, targetNumber, risk }) => {
    if (!account) {
      sendMessageToGame({
        type: 'BET_FAILED',
    ...
    console.log('🎡 Processing wheel bet:', { betAmount, targetNumber, risk });

    setIsPlaying(true);

    try {
      const response = await fetch(`${API_URL}/api/wheel/play`, {
        method: 'POST',
      ...
      sendMessageToGame({
        ...
        data: {
          ...
          payout: result.payout / 1_000_000_000, // Convert back to SUI
          targetNumber: result.targetNumber,
          resultNumber: result.resultNumber,
          newBalance: result.newBalance / 1_000_000_000
        }
      });
      ...
      console.error('❌ Wheel bet failed:', error);
      sendMessageToGame({
        type: 'BET_FAILED',
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <div className="bg-[#1C2455] rounded-xl p-4 text-sm text-gray-400">
        <p>🎡 <strong>Pick a number (1-10) and spin!</strong> Win up to 10x your bet on the Sui blockchain!</p>
      </div>
    </div>
  );
}